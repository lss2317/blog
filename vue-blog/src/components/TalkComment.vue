      ...
      >
        ...
        <div class="comment-meta">
          ...
          <div class="comment-user">
            ...
            <v-icon style="margin-left: 5px;" size="20" color="#ffa51e" v-if="item.userId === postUserId">
              mdi-check-decagram
            </v-icon>
          </div>
          <!-- 信息 -->
          ...
          >
            ...
            <div class="reply-meta">
              ...
              <div class="comment-user">
                ...
                <v-icon style="margin-left: 5px" size="20" color="#ffa51e" v-if="reply.userId === postUserId">
                  mdi-check-decagram
                </v-icon>
              </div>
              <!-- 信息 -->
              ...
              <p class="comment-content">
                ...
                <template v-if="reply.replyUserId !== item.userId">
                  <span class="comment-nickname ml-1">
                    @{{ reply.replyNickname }}
                  </span>
                  ，
                </template>
          ...
          >
            <span style="padding-right:10px">
              共{{ Math.ceil(item.replyCount / size) }}页
            </span>
            <paging
                ref="page"
                :totalPage="Math.ceil(item.replyCount / size)"
                :index="index"
                :commentId="item.id"
...
export default {
  ...
  methods: {
    ...
    checkReplies(index, item) {
      this.axios
          .get("/api/comment/" + item.id + "/replies", {
    ...
    changeReplyCurrent(current, index, commentId) {
      ...
      this.axios
          .get("/api/comment/" + commentId + "/replies", {
            params: {current: current, size: this.size}
          })
          .then(res => {
    ...
    listComments() {
      ...
      this.axios
          ...
          .then(res => {
            ...
            this.$emit("getCommentCount", res.data.data.commentTotal);
          });
    },
    insertComment() {
      //判断登录
    ...
    reloadReply(index) {
      this.axios
          .get("/api/comment/" + this.commentList[index].id + "/replies", {
            params: {
